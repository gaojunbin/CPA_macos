# Feature and integration reference

[Project overview](../README.md) · [Development](DEVELOPMENT.md)

## Connection setup

CLIProxyAPI must already be running on your server. Enable remote management with `remote-management.allow-remote: true` and configure `remote-management.secret-key` or `MANAGEMENT_PASSWORD`. Use HTTPS for an internet-facing server.
...
## Monitoring and quota semantics
...
- The menu bar shows the healthy/total account and channel ratio for the currently selected service. Switching shows that service's last-loaded data instantly while a fresh refresh runs in the background.
...
Click **•••** → **模型列表…** to inspect models advertised by the current service using its management key. This is an inventory of account and config-channel models, not a live inference check.
...
Download the DMG from [GitHub Releases](https://github.com/gaojunbin/CPA_macos/releases/latest), open it, and drag CPA into Applications. The app appears in the menu bar.

Release builds are ad-hoc signed and are not notarized. If macOS blocks first launch, verify that the download came from this repository, then use System Settings > Privacy & Security > Open Anyway.
